(ns secgov_edgar.main
  "Kotodama WASM entrypoint for the Secgov_edgar clean-room actor (L4) — Clojure port.
  ...
  facts (`secgov_edgar.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "secgov_edgar")
(def tier "L4")
...
(def entity-specs
  [{:entity "Filing"      :plural "filings"      :id-prefix "secgoved_fil"
    :fields [:filerId :form :period :status]     :required [:form :period]
    :coerce {}                       :refs {}}
   {:entity "Entity"      :plural "entities"     :id-prefix "secgoved_ent"
    :fields [:name :ein :jurisdiction]           :required [:name :ein]
    :coerce {}                       :refs {}}
   {:entity "Dataset"     :plural "datasets"     :id-prefix "secgoved_dat"
    :fields [:name :agency :updatedAt]           :required [:name :agency]
    :coerce {}                       :refs {}}
   {:entity "Application" :plural "applications" :id-prefix "secgoved_app"
    :fields [:applicantId :program :status]      :required [:program :status]
    :coerce {}                       :refs {}}
   {:entity "PublicRecord" :plural "publicrecords" :id-prefix "secgoved_pub"
    :fields [:recordType :jurisdiction :recordedAt] :required [:recordType :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Notice"      :plural "notices"      :id-prefix "secgoved_not"
...
(defn emit-facts
  "EAVT facts for one record: {\"secgov_edgar.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "secgov_edgar-compat" :tier tier :entities entities} 200])
...
(defn start! [] :secgov_edgar-compat/ready)

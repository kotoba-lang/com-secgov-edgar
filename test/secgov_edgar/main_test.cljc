(ns secgov_edgar.main-test
  "Contract + behavioral test for the secgov_edgar-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "secgov_edgar-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'secgov_edgar.main-test)]
